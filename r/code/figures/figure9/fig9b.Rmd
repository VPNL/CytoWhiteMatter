...
n_subs <- length(unique(foo$subject))
  ...
  df %>% 
    ...
    ylim(0,100) +
    ggtitle(roiname) +
    annotate("text",x=1.25, y=90, label=paste0("n=",n_subs),size = 6)


# save 
ggsave(filename = 
           paste0("fig9b_",roiname,"_",agegroup,".png"),
        width = 4,
         height = 3)

ggsave(filename = 
           paste0("fig9b_",roiname,"_",agegroup,".pdf"),
        width = 4,
         height = 3)
}
```
...
```{r}
roinames = c("mOTS-words","mFus-faces","OTS-bodies","CoS-places")
groups = c("kid","adult")

for (r in roinames){
  for (g in groups){
      func_fascicle_plot_single_roi(df,"lh",g,r)
  }
  }
```